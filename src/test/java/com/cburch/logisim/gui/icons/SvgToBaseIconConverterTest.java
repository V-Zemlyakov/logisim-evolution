...
public class SvgToBaseIconConverterTest {
  ...
  void testConvertSvgToBaseIcon() throws Exception {
    var svgPath = System.getProperty("svgPath");
    var javaPath = System.getProperty("javaPath");
    var packageName = System.getProperty("packageName");
    var className = System.getProperty("className");

    if (svgPath == null) svgPath = System.getProperty("org.gradle.project.svgPath");
    if (javaPath == null) javaPath = System.getProperty("org.gradle.project.javaPath");
    if (packageName == null) packageName = System.getProperty("org.gradle.project.packageName");
    if (className == null) className = System.getProperty("org.gradle.project.className");

    if (svgPath == null) svgPath = System.getProperty("systemProp.CONVERT_SVG_PATH");
    if (javaPath == null) javaPath = System.getProperty("systemProp.CONVERT_JAVA_PATH");
    if (packageName == null) packageName = System.getProperty("systemProp.CONVERT_PKG_NAME");
    if (className == null) className = System.getProperty("systemProp.CONVERT_CLS_NAME");

    if (svgPath == null) svgPath = System.getenv("CONVERT_SVG_PATH");
    if (javaPath == null) javaPath = System.getenv("CONVERT_JAVA_PATH");
    if (packageName == null) packageName = System.getenv("CONVERT_PKG_NAME");
    if (className == null) className = System.getenv("CONVERT_CLS_NAME");
  ...
  private static record IconMapping(String svgPath, String javaPath, String packageName, String className) {}

  @Test
  void testBatchReconvertAllIcons() throws Exception {
    final var isBatchRun = "true".equalsIgnoreCase(System.getProperty("reconvertAll"))
        || "true".equalsIgnoreCase(System.getenv("RECONVERT_ALL"));

    final var mappings = List.of(
        new IconMapping("src/main/resources/resources/logisim/icons/switch.svg", "src/main/java/com/cburch/logisim/std/io/extra/SwitchIcon.java", "com.cburch.logisim.std.io.extra", "SwitchIcon"),
        new IconMapping("src/main/resources/resources/logisim/icons/buzzer.svg", "src/main/java/com/cburch/logisim/std/io/extra/BuzzerIcon.java", "com.cburch.logisim.std.io.extra", "BuzzerIcon")
    );

    if (isBatchRun) {
      for (final var m : mappings) {
        final var svgFile = new File(m.svgPath());
        final var javaFile = new File(m.javaPath());
        if (svgFile.exists()) {
          System.out.println("Batch re-converting [" + m.svgPath() + "] -> [" + m.javaPath() + "]");
          convertAndWriteFile(svgFile, javaFile, m.packageName(), m.className());
        }
      }
    }
  }
